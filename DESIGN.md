...
### Progressive Selection -> Booking Handoff
...
- **Commitment/Hold:** The exploratory orbit deliberately collapses into a focused, single-pane glass sheet to reduce cognitive load while price and inventory are revalidated.
- **Booking Handoff:** Voyage Canvas ends with a clearly labeled `Continue to secure checkout` action. Payment is handled by the existing checkout outside AI authority; do not orbit payment/security controls inside the assistant.
...
## Implementation Addendum - Locked Voyage Canvas UX Rules

These rules resolve the final implementation decisions and override conflicting visual-prototype details.

### 1. Orbit is semantic, not decorative

Use orbit during:

- intent/constraint materialization
- exploration
- direct manipulation
- compare/trade-off decisions

Orbit represents relationships around the stable traveler context. Do not add orbital motion simply for visual novelty.

Collapse toward a conventional focused transactional layout during:

- authentication
- hold confirmation
- signed booking handoff

Do **not** orbit payment methods, encryption labels, or checkout controls.

### 2. Guest language vs architecture language

Internal component/domain names may include `TravelerCore`, `JourneyOrbit`, and `Evidence`. Guest-facing labels should remain simple, for example:
...
Do not expose architecture jargon such as agent, RAG, schema, ToolResult, or grounding to the guest.

### 3. Anonymous-first

The first experience must not imply a known named family/loyalty profile before authentication. Use generic traveler counts/icons during anonymous planning. Personalization may appear only after simulated sign-in if needed for the demo.

### 4. Intent-first entry

The initial hero invites natural language, e.g.:

> Tell us the trip you're imagining.

A prompt such as "7-night Caribbean cruise in March 2027 for 2 adults and 2 kids, balcony, under $5,000" should materialize the traveler/constraint model rather than forcing a separate traditional form first.

### 5. Evidence is visually distinct from AI interpretation

Commerce-sensitive surfaces use explicit deterministic evidence language:

- VERIFIED PRICE
- LIVE AVAILABILITY
- checked/as-of time
- valid-until time where applicable
- source/service label

AI fit/explanation language is visually distinct and may not masquerade as authoritative commerce data.

### 6. Geography is data-driven

Generated artwork may be used as atmospheric inspiration only. Real map labels/ports/positions used in the product come from trusted local structured data. Never display AI-generated/fabricated geographic names as authoritative UI.

### 7. Streaming shows operational progress

Prefer semantic progress states over theatrical character-by-character streaming:
...
Never stream an unverified price or availability claim.

### 8. Uncertainty uses product states, not fake confidence percentages
...
- This quote needs to be reverified
- I can't verify this right now

Do not show model confidence percentages.

### 9. Accessibility is mandatory

- every orbit node is a semantic keyboard-operable control
- Orbit and List views represent the same state
...
### 10. Runtime assets are local before demo freeze

Do not depend on prototype `googleusercontent` URLs or other uncontrolled external image hosts in the final hero/fallback path. Move required imagery/assets into `apps/web/public/assets/`.

### 11. UI references are not source code

HTML prototypes under `docs/references/ui/` are composition references only. Rebuild reusable components with React/TypeScript and shared domain contracts. Prototype errors/old dates/excessive navigation must not be carried forward.
